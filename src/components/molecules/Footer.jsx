        ...
        <footer className='bg-[#000] text-[#fff]'>
            <nav className='flex flex-wrap items-start container py-[70px] gap-[30px] lg:gap-[15px] xl:gap-[30px]'>
                ...
                <div className='w-[calc((100%-30px)/2)] md:w-[calc((100%-60px)/3)] lg:w-[calc((100%-60px)/4)] xl:w-[calc((100%-90px)/4)] '>
                    ...
                    <ul>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("about")}</Link></li>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("services")}</Link></li>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("projects")}</Link></li>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("comments")}</Link></li>
                    </ul>
                </div>
                <div className=' w-[calc((100%-30px)/2)] md:w-[calc((100%-60px)/3)] lg:w-[calc((100%-60px)/4)] xl:w-[calc((100%-90px)/4)] '>
                    <h3 className='text-[20px] font-semibold mb-[20px]'>İnformasiya</h3>
                    <ul>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("blogs")}</Link></li>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("contact")}</Link></li>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("career")}</Link></li>
                        <li><Link className='text-[14px] inline-block py-[8px]'>{t("news")}</Link></li>
                    </ul>
                </div>
                <div className=' w-[calc((100%-30px)/2)] md:w-[calc((100%-60px)/3)] lg:w-[calc((100%-60px)/4)] xl:w-[calc((100%-90px)/4)] '>
                    <h3 className='text-[20px] font-semibold mb-[20px]'>Dil seçimi</h3>
                    <ul>
                        <li onClick={() =>  handleLang('az') } className={` ${i18n.language === "az" && "active"} py-[8px] footer-lang text-[14px]`}>Azərbaycanca</li>
                        <li onClick={() => handleLang("en")} className={` ${i18n.language === "en" && "active"} py-[8px] footer-lang text-[14px]`}>English</li>
                        <li li onClick={() => handleLang("ru")} className={`${i18n.language === "ru" && "active"} py-[8px] footer-lang text-[14px]`}>Русский</li>
                    </ul>
                </div>
            </nav>
            <div></div>
